...
function calculate(){
  ...
  document.getElementById("totalContributions").textContent =
    money.format(totalContributions - initialDeposit);
...
function drawGrowthChart(data, initialDeposit){
  ...
  data.forEach((item, index) => {
    ...
    const contributionsOnly =
      Math.max(0, item.totalContributions - initialDeposit);

    const interestOnly =
      Math.max(0, item.balance - item.totalContributions);

    const initialHeight =
      (initialDeposit / maxValue) * chartHeight;

    const contributionHeight =
      (contributionsOnly / maxValue) * chartHeight;

    const interestHeight =
      (interestOnly / maxValue) * chartHeight;
    ...
    ctx.fillRect(
      barX,
      Math.round(baseY - initialHeight),
      cleanBarWidth,
    ...
    ctx.fillRect(
      barX,
      Math.round(baseY - initialHeight - contributionHeight),
      cleanBarWidth,
      Math.round(contributionHeight)
    );
