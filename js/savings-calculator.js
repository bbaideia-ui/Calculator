...
function calculate(){
  ...
  if(initialDeposit < 0 || annualContributionInput < 0 || monthlyContributionInput < 0 || annualRate < 0 || years <= 0 || taxRate < 0){
  ...
  drawBreakdownChart(initialDeposit, totalContributions - initialDeposit, totalInterest);
}
...
function drawGrowthChart(data, initialDeposit){
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(Math.round(padding), Math.round(padding));
  ctx.lineTo(Math.round(padding), Math.round(height - padding));
  ctx.lineTo(Math.round(width - padding), Math.round(height - padding));
  ctx.stroke();
  data.forEach((item, index) => {
    ...
    const cleanBarWidth = Math.round(barWidth);
    const initialY = Math.round(baseY - initialHeight);
    const initialH = Math.round(initialHeight);
    const contributionY = Math.round(baseY - initialHeight - contributionHeight);
    const contributionH = Math.round(contributionHeight);
    const interestY = Math.round(baseY - initialHeight - contributionHeight - interestHeight);
    const interestH = Math.round(interestHeight);
    ctx.fillStyle = isDark ? "#2f81f7" : "#0969da";
    ctx.fillRect(barX, initialY, cleanBarWidth, initialH);
    ctx.fillStyle = isDark ? "#3fb950" : "#2da44e";
    ctx.fillRect(barX, contributionY, cleanBarWidth, contributionH);
    ctx.fillStyle = isDark ? "#f85149" : "#cf222e";
    ctx.fillRect(barX, interestY, cleanBarWidth, interestH);
  });
...
function drawBreakdownChart(initialDeposit, contributions, interest){
  ...
  const parts = [
    {value: initialDeposit, color: isDark ? "#2f81f7" : "#0969da"},
  ...
  const legendY = 218;
  ctx.fillStyle = isDark ? "#2f81f7" : "#0969da";
  ctx.fillRect(Math.round(legendX), Math.round(legendY), 10, 10);
  ctx.fillStyle = isDark ? "#8b949e" : "#57606a";
  ctx.fillText("Initial", legendX + 16, legendY + 10);
  ctx.fillStyle = isDark ? "#3fb950" : "#2da44e";
  ctx.fillRect(Math.round(legendX + 85), Math.round(legendY), 10, 10);
  ctx.fillStyle = isDark ? "#8b949e" : "#57606a";
  ctx.fillText("Contributions", legendX + 101, legendY + 10);
  ctx.fillStyle = isDark ? "#f85149" : "#cf222e";
  ctx.fillRect(Math.round(legendX + 205), Math.round(legendY), 10, 10);
  ctx.fillStyle = isDark ? "#8b949e" : "#57606a";
